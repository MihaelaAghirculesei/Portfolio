...
const ALLOWED_ORIGIN = 'https://aghirculesei.pages.dev';
...
export default {
  async fetch(request, env) {
    ...
    if (!emailRegex.test(email)) {
      return corsResponse(JSON.stringify({ error: 'Invalid email address' }), 400, env);
    }
    ...
    try {
      ...
      if (!resendResponse.ok) {
        ...
        return corsResponse(JSON.stringify({ error: 'Failed to send email' }), 500, env);
      }

      return corsResponse(JSON.stringify({ success: true }), 200, env);

    } catch (err) {
      console.error('Worker error:', err);
      return corsResponse(JSON.stringify({ error: 'Internal server error' }), 500, env);
    }
  },
};
